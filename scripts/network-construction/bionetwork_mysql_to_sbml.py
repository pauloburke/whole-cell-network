...
#Retrieving network vertices from database -------------
	...
	createMoleculeVertice(network,TERMINAL_ORGANELLE,'Terminal Organelle','Organelle')
	createMoleculeVertice(network,MEMBRANE,'Membrane','Membrane')
			...
			if len(reactants)>0:
				...
				for molecule in reactants:
					...
					if compartment == 'e':
						...
						molecule['name'] = molecule['name']+' extracell'
						createModifier(r,MEMBRANE)
					if molecule['wid'].replace("-","_") not in speciesList:
						createMoleculeVertice(network,molecule['wid'].replace("-","_"),molecule['name'],"Other")
...
#DNA Replication --------------------------------------------------------------
	...
	createMoleculeVertice(network,REPLICATION_CR,'Chromosome ready to replication','Chromosome')

	#Replication iniciation reaction
	counter += 1
	r = network.createReaction()
	check(r)
	check(r.setId('REPLICATION_INICIATION'))
	check(r.setName('Replication Iniciation'))
	check(r.setReversible(False))
	check(r.setFast(False))
	...
	createModifier(r,SSBINDPROT) #Checar esse numero
	...
	#Create new DNAs
	createMoleculeVertice(network,DUPLICATED_CR,'Duplicated Chromosome','Chromosome')
	createProduct(r,DUPLICATED_CR,2)


	#DNA Damage
	
	createMoleculeVertice(network,DAMN_CR,'Damaged DNA','Chromosome')
	damageTypes = mysqlQuery(c,"select public_entry.wid from public_speciescomponent_type inner join public_entry on public_entry.id=public_speciescomponent_type.type_id where public_speciescomponent_type.speciescomponent_id IN (select parent_ptr_species_component_id from public_reaction where processes_id IN(select public_entry.id from public_process inner join public_entry on public_entry.id = public_process.parent_ptr_species_component_id where public_entry.wid='Process_DNADamage')) group by public_entry.wid")
	for damnType in damageTypes:
		counter += 1
		r = network.createReaction()
		check(r)
		check(r.setId('DNA_DAMAGE_'+damnType['wid'].replace("-","_")))
		check(r.setName('DNA damage '+damnType['wid'].replace("-","_")))
		check(r.setReversible(False))
		check(r.setFast(False))
		check(r.appendAnnotation('Replication Reaction DNA Damage'))
		check(r.setNotes('Replication',True))

		createReactant(r,DUPLICATED_CR,1)
		reactions = mysqlQuery(c,"select public_entry.wid from public_speciescomponent_type inner join public_entry on public_entry.id = public_speciescomponent_type.speciescomponent_id where type_id IN (select public_entry.id from public_entry where public_entry.wid='"+damnType['wid']+"')")
		for damnReact in reactions:
			damnReact = damnReact['wid']
			dmR = network.getReaction(damnReact)
			bases = []
			bases.append(damnReact.split('_')[1])
			if len(damnReact.split('_')) > 2:
				bases.append(damnReact.split('_')[2])
			if dmR!=None:
				modN = dmR.getNumModifiers()
				for i in range(modN):
					mod = dmR.getModifier(long(i))
					if mod!=None:
						createModifier(r,mod.getSpecies())
				reactN = dmR.getNumReactants()
				for i in range(reactN):
					reactant = dmR.getReactant(long(i))
					if reactant!=None and reactant.getSpecies() not in bases:
						createReactant(r,reactant.getSpecies(),reactant.getStoichiometry())
				prodN = dmR.getNumProducts()
				for i in range(reactN):
					product = dmR.getProduct(long(i))
					if product!=None and product.getSpecies() not in bases:
						createProduct(r,product.getSpecies(),product.getStoichiometry())
				network.removeReaction(damnReact)
		createProduct(r,DAMN_CR,1)
	...
	r = network.createReaction()
	check(r)
	check(r.setId('DNA_REPAIR'))
	check(r.setName('Chromosome Repair'))
	check(r.setReversible(False))
	check(r.setFast(False))
	check(r.appendAnnotation('Replication Reaction'))
	check(r.setNotes('Replication',True))
	...
	r = network.createReaction()
	check(r)
	check(r.setId('REPLICATION_SEGREGATION'))
	check(r.setName('Replication Segregation'))
	check(r.setReversible(False))
	check(r.setFast(False))
	check(r.appendAnnotation('Replication Reaction'))
	check(r.setNotes('Replication',True))
	...
	#Create new Chromosomes
	createMoleculeVertice(network,CR,'Chromosome','Chromosome')
	createProduct(r,CR,1)

	#Create uncondensation reaction
	counter += 1
	r = network.createReaction()
	check(r)
	check(r.setId('REPLICATION_END'))
	check(r.setName('Replication End'))
	check(r.setReversible(False))
	check(r.setFast(False))
	check(r.appendAnnotation('Replication Reaction'))
	check(r.setNotes('Replication',True))
	#Add Chromosome
	createReactant(r,CR,1)
	#add Transcriptional Units	
	tUnits = mysqlQuery(c,'select public_entry.wid from public_transcriptionunit inner join public_entry on public_entry.id = public_transcriptionunit.parent_ptr_molecule_id')
	for tu in tUnits:
		createProduct(r,tu['wid'].replace("-","_"),1)
...
#Removing DNA Polymerization Old Reactions ------------------------------------
	...
	while i<maxi:
		if i<network.getNumReactions():
			r = network.getReaction(long(i))
			if r != None:
				if DNABINDREACT_PREFIX in r.getId() or DNAPRIMERPOLY_PREFIX in r.getId() or DNAAPOLY_PREFIX2 in r.getId() or CHROMOSEGREGATION_PREFIX in r.getId() or FTSZPOLY_PREFIX in r.getId() or DNAAPOLY_PREFIX in r.getId():
					deletedReactions.append(r.getId())
					network.removeReaction(r.getId())
					counter = counter+1
					i = i-1
			...
			if len(genes)>1:
				...
				if processingFlag:
					...
					if mRNA not in speciesList:
						createMoleculeVertice(network,mRNA,tr['wid'].replace("-","_")+' Poly mRNA',"PolymRNA")
						speciesList.append(mRNA)
			else:
				mRNA = genes[0]['wid']
				if mRNA not in speciesList:
					createMoleculeVertice(network,mRNA,mRNA,'RNA')
					speciesList.append(mRNA)
			#Create mRNA
			...
			for gene in genes:
				if processingFlag:
					if gene['wid'] not in speciesList:
						createMoleculeVertice(network,gene['wid'],gene['wid'],'RNA')
...
#EF Reactions ---------------------------------------------------------------
	...
	for EF in EFs:
		...
		check(r.setNotes('Transciption',True))
		...
		if 'Aminoacylation' in reaction:
			...
			if len(reactants)>0:
				...
				if 'TRNA_'+aa not in speciesList:
					createMoleculeVertice(network,'TRNA_'+aa,'tRNA with '+aa,'tRNA_aminoacylated')
				...
				if aa != 'FMET':
					...
					if checkReaction == None:
						...
						createReactant(r,EFTU+'_'+GTP,1)
						if 'TRNA_'+aa+'_TU' not in speciesList:
							createMoleculeVertice(network,'TRNA_'+aa+'_TU','tRNA with '+aa+' and EFTU','tRNA_aminoacylated_EFTU')
			...
			if r == None:
				...
				if len(reactants)>0:
					...
					if 'PRE_'+modification not in speciesList:
						createMoleculeVertice(network,'PRE_'+modification,'PRE_'+modification,'pre RNA')
		...
		if proteinCount:
			...
			for geneName in polycistrons[mRNA]:
				...
				if gene != None:
					...
					for key in aaCounter:
						if aaCounter[key] > 0:
							...
							if 'TRNA_'+AMINOACIDS_INVERSE[key]+'_TU' not in speciesList:
								createMoleculeVertice(network,'TRNA_'+AMINOACIDS_INVERSE[key]+'_TU','tRNA with '+AMINOACIDS_INVERSE[key]+' with EFTU','tRNA_aminoacylated_EFTU')
					...
					#Add EFTUs and EFGs
					createProduct(r,EFTU+'_'+GDP,protSize)
...
#Complex formation reactions --------------------------------------------------
	...
	print 'Creating complex formation reactions...'
